...
const Filter = () => {
  ...
  const fetchBooksByGenres = async () => {
    ...
    try {
      ...
      const apiUrl = `http://localhost:1000/api/v1/get-books-by-genre?genre=${encodeURIComponent(
        sanitizedGenre
      )}`;
    ...
    <div className="p-6 bg-white text-black isolate">
      <div className="border rounded-lg p-12 mt-6">
          ...
          {genreRows.map((row, rowIndex) => (
            <div
              ...
              className={`grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-0 ${
                rowIndex % 2 === 0
                  ? "pt-1 pb-2 relative right-1"
                  : "pt-1 pb-2 relative left-19"
              }`}
            >
              {row.map((genre, index) => (
                <button
                  key={index}
                  className={`flex font-semibold items-center justify-center gap-2 w-[90%] py-3 ${
                    selectedGenres.includes(genre.name)
                      ? "bg-[#c87e70]"
                      : "bg-[#bdbcbb]"
                  } text-black text-xl rounded-full shadow-md hover:bg-[#c87e70] focus:outline-none`}
        ...
        {!loading && books && books.length > 0 ? (
          ...
          !loading && <p className="text-center text-gray-500 mt-6"></p>
        )}
      </div>
    </div>
  );
