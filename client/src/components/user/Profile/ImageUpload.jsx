...
const ImageUpload = ({ onImageSelect }) => {
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      // Corrected comment and alert to match 30MB
      if (file.size > 30 * 1024 * 1024) { // 30MB limit
        alert("Image size exceeds 30MB limit."); // Corrected alert message
        return;
      }
      const reader = new FileReader();
      reader.onloadend = () => {
        setImagePreview(reader.result);
        onImageSelect(reader.result); // Sends base64 string to parent
      };
