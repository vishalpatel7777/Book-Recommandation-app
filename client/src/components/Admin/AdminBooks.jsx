...
const AdminBooks = () => {
  ...
  useEffect(() => {
    // Only fetch books if we are on the base path
    if (isBasePath) {
  ...
  }, [isBasePath]); // Re-run if the path changes
        ...
        <div className="gap-4 flex w-full p-4 h-auto bg-gray-100 shadow-lg rounded-xl">
          ...
          <div className="w-10/12 bg-white p-6 shadow-lg rounded-r-xl overflow-y-auto">
            {isBasePath ? (
              ...
              <Outlet /> // This will render AddBook, EditBook (form), or DeleteBook
            )}
          </div>
        </div>
      )}
...
export default AdminBooks; // <-- Renamed export