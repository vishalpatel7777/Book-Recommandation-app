...
export default function AdminSettings() { // Renamed component
  ...
  const handleUpdateProfile = async () => {
    try {
      ...
      const response = await api.put(`/update-admin-profile`, updatedData, {
        headers,
      });
      setAdmin(response.data);
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      const response = await api.put(
        `/update-admin-profile`, // Assuming this is the correct endpoint for password change as well
        { oldPassword: passwords.oldPassword, password: passwords.newPassword },
        { headers }
      );
      setAlertMessage("Password updated successfully!");
      setPasswords({ oldPassword: "", newPassword: "" }); // Clear fields
      setShowAlert(true);
      setTimeout(() => setShowAlert(false), 2000);
    } catch (error) {
      setAlertMessage(
