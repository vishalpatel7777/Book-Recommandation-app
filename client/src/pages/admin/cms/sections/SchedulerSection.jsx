...
export default function SchedulerSection() {
  ...
  const [jobs, setJobs] = useState(MOCK_SCHEDULED);
  ...
  const deleteJob = (j) => { setJobs(prev => prev.filter(x => x.id !== j.id)); toast?.("Job removed"); };
      ...
      <div style={{ ...st.card, background: "rgba(59,130,246,0.04)", border: "1px solid rgba(59,130,246,0.15)", marginBottom: 16 }}>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
          ...
          <p style={{ fontSize: "0.78rem", color: "var(--text-muted)", lineHeight: 1.5 }}>
            Schedule publish, archive, or send actions for Books, Promotions, Homepage sections, and Notifications. Scheduled jobs run at the specified time in the selected timezone.
          </p>
        </div>
      </div>
